...
//constante pour les tailles des données à envoyer dans les messages MPI
#define COUNT 1023
...
{
    //création d'un nouveau type de donné MPI
    ...
    std::cout << "Début" << population.size()<< std::endl << std::flush;
    ...
    {   
        ...
        MPI_Recv(&quitting,1, MPI::BOOL, 0, 0, MPI_COMM_WORLD, &status); //1
        ...
        //Envoie des données pour l'affichage
        MPI_Ssend(&jours_écoulé,1, MPI::UNSIGNED_LONG, 0, 0, MPI_COMM_WORLD);  
        MPI_Ssend(&dim, 2, MPI_INT, 0, 0, MPI_COMM_WORLD);  
        MPI_Ssend(grill_stat.data(), COUNT_0, StatistiqueParCaseT, 0, 0, MPI_COMM_WORLD);
    ...
    sdl2::init(argc, argv);
    {
        ...
        if(rank==0)
        {
            ...
            while(!quitting)
            {
                ...
                MPI_Ssend(&quitting,1,MPI::BOOL,1,0,MPI_COMM_WORLD); //1

                unsigned long jours_écoulé;
                std::array<int,2> dim;
                std::vector<épidémie::Grille::StatistiqueParCase> grill_stat(COUNT_0);

                //Réception des données pour l'affichage
                MPI_Recv(&jours_écoulé,1,MPI::UNSIGNED_LONG,1,0,MPI_COMM_WORLD,&status); //3
